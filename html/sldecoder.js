...
Module["workerArrayYUV"] = new ArrayBuffer(1);
...
onmessage = function(event) {
    ...
    switch (event.data.m) {
        case "onWorkerConnect":
            ...
            break;
        case "onWorkerRequest":
            Module["onWorkerRequest"](event.data);
            break;
        case "onWorkerACK":
            Module["onWorkerACK"](event.data);
            break;
        default:
...
Module["doWorkerInit"] = function() {
    postMessage({
        "m": "onWorkerInit"
    })
};
Module["doWorkerConnect"] = function() {
    postMessage({
        "m": "onWorkerConnect"
    })
};
Module["doWorkerError"] = function(code, err) {
    postMessage({
        "m": "onWorkerError",
        "c": code,
        "e": err
...
Module["doWorkerYUV"] = function(width, height, len, ts) {
    postMessage({
        "m": "onWorkerYUV",
        "w": width,
        "h": height,
        "ts": ts,
        "yuv": Module["workerArrayYUV"],
        "len": len
    }, [Module["workerArrayYUV"]]);
    Module["workerArrayYUV"] = null
};
Module["doWorkerPCM"] = function(sample, bit, channels, pcm) {
    postMessage({
        "m": "onWorkerPCM",
        "s": sample,
        "b": bit,
...
Module["onWorkerConnect"] = function(obj) {
    Module["websocket"]["url"] = obj.url;
    this._onWorkerConnect()
};
Module["onWorkerACK"] = function(obj) {
    if ("yuv" === obj.name) {
        Module["workerArrayYUV"] = obj.value
    }
};
var moduleOverrides = {};
var key;
...
var STACK_BASE = 260208,
    DYNAMIC_BASE = 5503088,
    DYNAMICTOP_PTR = 259952;
...
var wasmBinaryFile = "sldecoder.wasm";
...
function _workerConnect() {
    if (Module && "function" == typeof Module["doWorkerConnect"]) {
        Module["doWorkerConnect"]()
    } else {
        console.error("not found doWorkerConnect function")
    }
}

function _workerError(code, err) {
    if (Module && "function" == typeof Module["doWorkerError"]) {
        Module["doWorkerError"](code, UTF8ToString(err))
    } else {
        console.error("not found doWorkerError function")
    }
}

function _workerPCM(sample, bit, channels, pcm, len) {
    if (Module && "function" == typeof Module["doWorkerPCM"]) {
        var buff = new Uint8Array(HEAP8.subarray(pcm, pcm + len));
        Module["doWorkerPCM"](sample, bit, channels, buff)
    } else {
        console.error("not found doWorkerPCM function")
    }
}

function _workerResponse(reqID, data, len) {
    if (Module && "function" == typeof Module["doWorkerResponse"]) {
        var buff = new Uint8Array(HEAP8.subarray(data, data + len));
        Module["doWorkerResponse"](reqID, buff)
    } else {
        console.error("not found doWorkerResponse function")
    }
}

function _workerYUV(width, height, yuv, len, ts) {
    if (Module && "function" == typeof Module["doWorkerYUV"]) {
        if (null === Module["workerArrayYUV"] || "object" != typeof Module["workerArrayYUV"]) {
            return
        }
        if (Module["workerArrayYUV"].byteLength < len) {
            Module["workerArrayYUV"] = new ArrayBuffer(Uint8Array.BYTES_PER_ELEMENT * len)
        }
        var buff = new Uint8Array(Module["workerArrayYUV"]);
        buff.set(HEAP8.subarray(yuv, yuv + len), 0);
        Module["doWorkerYUV"](width, height, len, ts)
    } else {
        console.error("not found doWorkerYUV function")
    }
}
__ATINIT__.push({
    func: function() {
...
var ___tm_timezone = (stringToUTF8("GMT", 260096, 4), 260096);
...
var asmLibraryArg = {
    ...
    "_workerPCM": _workerPCM,
    "_workerResponse": _workerResponse,
    "_workerYUV": _workerYUV,
...
var ___em_js__workerError = Module["___em_js__workerError"] = function() {
    return Module["asm"]["___em_js__workerError"].apply(null, arguments)
};
var ___em_js__workerPCM = Module["___em_js__workerPCM"] = function() {
    return Module["asm"]["___em_js__workerPCM"].apply(null, arguments)
};
var ___em_js__workerResponse = Module["___em_js__workerResponse"] = function() {
    return Module["asm"]["___em_js__workerResponse"].apply(null, arguments)
};
var ___em_js__workerYUV = Module["___em_js__workerYUV"] = function() {
    return Module["asm"]["___em_js__workerYUV"].apply(null, arguments)
};
...
var _onWorkerConnect = Module["_onWorkerConnect"] = function() {
    return Module["asm"]["_onWorkerConnect"].apply(null, arguments)
};
